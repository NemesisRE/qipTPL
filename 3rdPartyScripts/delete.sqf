...
_viewDist   = qipTPL_CleanUp_viewDist;   // min distance in meter from a player unit to allow delete, if you dont care if player sees the delete, set it to 0.
_manTimer   = qipTPL_CleanUp_manTimer;   // x seconds until delete of dead man units.
_vehTimer   = qipTPL_CleanUp_vehTimer;   // x seconds until delete of dead vehicles, for destroyed and heavy damaged vehicles.
_abaTimer   = qipTPL_CleanUp_abaTimer;   // x seconds a vehicle must be unmanned to be deleted, for _abandoned option.
