...
/*
	...
	ie. nul = [3,true] execVM "\qipTPL\3rdPartyAddons\MKY\MKY_Sand_Snow_Init.sqf";
*/
if (isNil "qipTPL_snowStormIntensity") then { qipTPL_snowStormIntensity = 0 };
MKY_iStrengthEFX = qipTPL_snowStormIntensity;
if ((count _this) > 0) then {if (typeName (_this select 0) == "SCALAR") then {MKY_iStrengthEFX = (_this select 0);};};

// vary EFX dropInterval ?
MKY_bVaryEFX = true; // default
