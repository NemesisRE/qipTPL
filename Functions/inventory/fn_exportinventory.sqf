...
switch _type do {
	...
	case "init": {
		...
		//--- Arsenal label
		if !(isnil {uinamespace getvariable "qipTPL_fnc_arsenal_display"}) then {
			if (true) then {
				private "_br";
				_br = "";
		...
		if (_identity) then {
			...
			_export = _export + format ["%1 setFace ""%2"";",_var,_center getvariable ["qipTPL_fnc_arsenal_face",face _center]] + _br;
