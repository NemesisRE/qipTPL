...
//--- Get cargo list
_cargo = _object getvariable ["qipTPL_addVirtualWeaponCargo_cargo",[[],[],[],[]]];
...
if (_save) then {
	if (typename _object == typename missionnamespace) then {
		_object setvariable ["qipTPL_addVirtualWeaponCargo_cargo",_cargo];
		publicvariable "qipTPL_addVirtualWeaponCargo_cargo";
	} else {
		_object setvariable ["qipTPL_addVirtualWeaponCargo_cargo",_cargo,_isGlobal];
	};
};
...
if (_initAction && typename _object == typename objnull) then {
	if ({count _x > 0} count _cargo > 0) then {
		//--- Init arsenal
		["AmmoboxInit",_object] call qipTPL_fnc_arsenal;
	} else {
		//--- Terminate arsenal
		["AmmoboxExit",_object] call qipTPL_fnc_arsenal;
	};
};
